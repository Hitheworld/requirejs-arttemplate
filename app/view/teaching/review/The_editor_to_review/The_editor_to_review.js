...
define(['template',
        ...
        'layer',
        'laypage',
        'text!tplUrl/teaching/review/The_editor_to_review/The_editor_to_review.html',
    ...
    function (template,$,layui,layer,laypage,
            ...
            function CreareviewObj(){
                ...
                this.getZTreeName = function(event, treeId, treeNode){
                    ...
                    if(  !treeNode.checked ){
                        ...
                        _seft.appendSubTpl(treeNode.id);
                 ...
                 * 获取知识资源
                 */
                this.getKResources = function(typeId, kp_id){
                    kpointIds = kpointIds.toString();

                    //判断是否是常见问题---常见问题要每页面显示3个
                    if ( typeId == 7 ){
                        var findCourseResourceDB = findCourseResource(kp_id, typeId, COURSERESOURCE_APPLY, COURSERESOURCE_PAGENUMBER, COURSERESOURCE_PAGESIZE_RICHTEXT);
                    }else{
                        var findCourseResourceDB = findCourseResource(kp_id, typeId, COURSERESOURCE_APPLY, COURSERESOURCE_PAGENUMBER, COURSERESOURCE_PAGESIZE);
                    }
                ...
                this.allToLoadMore = function( re_type, kp_id ){
                    ...
                    // 分页
                    var modeldb = _seft.getKResources(re_type , kp_id);  //数据源
                    if( re_type == 0 ){
                     ...
                     * @param domid
                     */
                    function pageing (pageClass,domid, totalPage, callback){
                        laypage({
                            cont: $(pageClass+domid), //容器。值支持id名、原生dom对象，jquery对象,
                            pages: totalPage, //总页数
                            groups: 0, //连续分数数0
                            prev: false, //不显示上一页
                            next: '更多&gt;',
                ...
                this.interaction = function(){
                    ...
                    $("#createtestForm").validate({
                        ...
                        submitHandler: function(){
                            ...
                            if( kpointIds!= '' ){
                                var saveReviewDB = saveReview( reviewName, squadId, courseId, kpointIds, publish );
                                if(saveReviewDB.success){
                                    ...
                                    var index = layer.alert(markedWords ,function(){
                                        layer.close(index);
                                        setTimeout(function(){
                                            location.href = "#/teaching/review/reviewhome";
                                        },1000);
                                    });
                                }else {
                ...
                this.popup = function(kpid){
                    //资源数据
                    $('.tab-title-'+kpid+' li').on('click', function(){
                        //获取知识点下的资源
                ...
                this.clickInter = function( re_type ) {
                    ...
                    $('.J-view-video').on('click', function(){
                        ...
                        layer.open({
                            ...
                            content: template.compile( videoTpl)({
                                videoUrl:_seft.getKResources(COURSERESOURCE_TYPEID, kp_id)
                            })
                            //content: '<div id="video"></div>'
                        });
                        //playvideo(id); //调用视频
                    ...
                    //音频
                    $('.J-view-audio').on('click', function(){
                    ...
                    //图片
                    $('.J-view-pic').on('click', function(){
                        $('.J-view-pic').removeClass('avite');
                        $(this).addClass('avite');
                        var url = $(this).data('url');
                        //iframe层-父子操作
                        layer.open({
                    ...
                    //富文本
                    $('.J-view-richtext').on('click', function(){
                        ...
                        layer.open({
                            ...
                            content:  template.compile( richtextTpl)({
                                problems: _seft.getKResources(re_type, kp_id)   //课程
                            })
                        });
                    });
        ...
        //查询课程对应的章节知识点树
        var courseChapterPointTree = function(pointId) {
        ...
        var findCourseResource = function(treeId, type, apply, pageNumber, pageSize) {
            return common.requestService('bxg/courseResource/findPage','get', {
                treeId: treeId,
                type: type,
                apply: apply,
        ...
        //保存发布 添加复习
        var saveReview = function( reviewName, squadId, courseId, pointIds , publish) {
